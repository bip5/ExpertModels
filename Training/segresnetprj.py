...
from Training.segresnet_blockprj import ResBlock, get_conv_layer, get_upsample_layer, modified_get_conv_layer,modified_ResBlock
from monai.networks.layers.factories import Dropout
from monai.networks.layers.utils import get_norm_layer
from monai.utils import UpsampleMode
from Training.CustomActivation import get_act_layer
__all__ = ["SegResNet", "SegResNetVAE","SegResNetAtt"]
...
class SegResNetAtt(SegResNet):
    ...
    def __init__(
        ...
        self.upsample_mode = UpsampleMode(upsample_mode)
        self.use_conv_final = use_conv_final
        self.convInit = modified_get_conv_layer(spatial_dims, in_channels, init_filters)
        self.down_layers = self._make_down_layers()
        self.up_layers, self.up_samples = self._make_up_layers()
        self.conv_final = self._make_final_conv(out_channels)
    ...
    def _make_down_layers(self):
        ...
        for i, item in enumerate(blocks_down):
            layer_in_channels = filters * 2**i
            pre_conv = (
                modified_get_conv_layer(spatial_dims, layer_in_channels // 2, layer_in_channels, stride=2)
                if i > 0
                else nn.Identity()
            )
            down_layer = nn.Sequential(
                pre_conv, *[modified_ResBlock(spatial_dims, layer_in_channels, norm=norm, act=self.act) for _ in range(item)]
            )
            down_layers.append(down_layer)
        return down_layers
    ...
    def _make_up_layers(self):
        ...
        for i in range(n_up):
            ...
            up_layers.append(
                nn.Sequential(
                    *[
                        modified_ResBlock(spatial_dims, sample_in_channels // 2, norm=norm, act=self.act)
                        for _ in range(blocks_up[i])
                    ]
                )
            )
            up_samples.append(
                nn.Sequential(
                    *[
                        modified_get_conv_layer(spatial_dims, sample_in_channels, sample_in_channels // 2, kernel_size=1),
                        get_upsample_layer(spatial_dims, sample_in_channels // 2, upsample_mode=upsample_mode),
                    ]
                )
            )
